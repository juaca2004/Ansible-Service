...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "serverPrincipal" do |serverPrincipal|
    ...
    serverPrincipal.vm.hostname = "serverPrincipal"
	  serverPrincipal.vm.network "public_network", ip: "192.168.1.170"
...
EOF
  ...
  config.vm.define "serverSecundario" do |serverSecundario|
    ...
    serverSecundario.vm.network "public_network", ip: "192.168.1.171"
    ...
    serverSecundario.vm.provision "shell", inline: <<-SHELL
      ...
      sudo apt install python3-pip -y
      pip3 install PyMySQL
    SHELLNETPLAN_FILE="/etc/netplan/01-netcfg.yaml"
    sudo bash -c "cat > $NETPLAN_FILE" << EOF
network:
  version: 2
  renderer: networkd
...
EOF
  ...
  config.vm.define "webserver" do |webserver|
    ...
    webserver.vm.network "public_network", ip: "192.168.1.202"
